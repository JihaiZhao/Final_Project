...
int main(int argc, char** argv) {
  ...
  try {
    ...
    // Stiffness
    const std::array<double, 7> k_gains = {{80.0, 80.0, 80.0, 80.0, 12.0, 12.0, 2.0}};
    // Damping
    const std::array<double, 7> d_gains = {{6.5, 6.5, 6.5, 6.5, 2.0, 2.0, 0.65}};
    ...
    std::function<franka::Torques(const franka::RobotState&, franka::Duration)>
      ...
      auto joint_positions_master = node->get_joint_position();

      std::array<double, 7>joint_array{};
      std::copy(joint_positions_master.begin(), joint_positions_master.end(), joint_array.begin());

      auto joint_velocity = node->get_joint_velocity();
      std::array<double, 7> joint_velocity_array;
      std::copy(joint_velocity.begin(), joint_velocity.end(), joint_velocity_array.begin());

      std::array<double, 7> tau_d_calculated;
      // std::cout << "diff in q_d, q = ";
      for (size_t i = 0; i < 7; i++) {
        tau_d_calculated[i] =
            // k_gains[i] * (state.q_d[i] - state.q[i]) - d_gains[i] * state.dq[i] + coriolis[i];
            k_gains[i] * (joint_array[i] - state.q[i]) - d_gains[i] * state.dq[i] + coriolis[i];
            // k_gains[i] * (joint_array[i] - state.q[i]) - d_gains[i] * joint_velocity_array[i] + coriolis[i];
        // std::cout << joint_array[i] - state.q[i] << " ";
        // std::cout << joint_velocity_array[i] << " ";
      }
      // std::cout << std::endl;
