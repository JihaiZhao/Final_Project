...
public:
    ...
    void publish_joint_state(const Eigen::VectorXd& q) {
...
int main(int argc, char** argv) {
  ...
  try {
    ...
    std::function<franka::Torques(const franka::RobotState&, franka::Duration)>
      ...
      // // calculate the torque at end-effector (the torque generated when push the master robot)
      // Eigen::VectorXd tau_(1);
      // tau_ee << jacobian.transpose() * tau_J;
      // std::cout << "tau at ee " << tau_J << std::endl;
      ...
      //   tau_d_array[i] -= 0.5 * tau_slave_array[i];
      // }

      // save data to csv
      csv_torque(robot_state, tau_d_array, coriolis_array, jacobian);

      node->publish_joint_state(q);
